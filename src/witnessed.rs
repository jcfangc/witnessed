...
mod impls {
    ...
    mod try_new_tests {
        ...
        impl Witness<i32> for Pos {
            type Error = PosErr;
            fn verify(input: &i32) -> Result<(), Self::Error> {
                (*input > 0).then_some(()).ok_or(PosErr::NonPos)
            }
        }
        ...
        impl Witness<u8> for CountOnce {
            ...
            fn verify(input: &u8) -> Result<(), Self::Error> {
                CALLS.fetch_add(1, Ordering::Relaxed);
                (*input == 1).then_some(()).ok_or(CountErr::Nope)
            }
        }
...
mod impl_fors {
    ...
    mod witness_debug_tests {
        ...
        fn debug_fmt_preserves_inner_debug_repr_exactly() {
            let w = Witnessed::<&'static str, Any>::try_new("hi").unwrap();
            let s = format!("{:?}", w);
        ...
        fn debug_fmt_complex_structure() {
            let w = Witnessed::<Vec<i32>, Any>::try_new(vec![1, 2]).unwrap();
    ...
    mod witness_ord_tests {
        ...
        fn ord_cmp_delegates_to_inner_and_sort_matches_inner_sort() {
            let mut v = [
                Witnessed::<i32, Any>::try_new(3).unwrap(),
                Witnessed::<i32, Any>::try_new(1).unwrap(),
                Witnessed::<i32, Any>::try_new(2).unwrap(),
            ];
            v.sort(); // uses Ord::cmp
        ...
        fn partial_ord_propagates_none_for_nan_like_inner() {
            let nan = Witnessed::<f32, Any>::try_new(f32::NAN).unwrap();
            let one = Witnessed::<f32, Any>::try_new(1.0).unwrap();
...
mod witnessed_size_tests {
    ...
    impl Witness<Vec<u8>> for AnyVec {
        ...
        fn verify(input: &Vec<u8>) -> Result<(), Self::Error> {
            if input.is_empty() {
                Err(VecErr::EmptyVec)
            } else {
                Ok(())
